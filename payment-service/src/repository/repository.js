    ...
    return new Promise((resolve, reject) => {
      ...
      stripe.charges.create({
        ...
        if (err && err.type === 'StripeCardError') {
          reject(new Error('An error occured procesing payment with stripe, err: ' + err))
        } else {
      ...
      makePurchase(payment)
        .then(paid => {
          db.collection('payments').insertOne(paid, (err, result) => {
            if (err) {
              reject(new Error('an error occured registering payment at db, err:' + err))
            }
            resolve(paid)
          })
        })
  ...
  const getPurchaseById = (paymentId) => {
    return new Promise((resolve, reject) => {
      const response = (err, payment) => {
        if (err) {
          reject(new Error('An error occured retrieving an order, err: ' + err))
        }
        resolve(payment)
      }
