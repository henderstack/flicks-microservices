  ...
  const makeBooking = (user, booking) => {
    return new Promise((resolve, reject) => {
      ...
      db.collection('booking').insertOne(payload, (err, booked) => {
        if (err) {
          reject(new Error('An error occuered registering a user booking, err:' + err))
        }
        resolve(payload)
      })
    })
  ...
  const generateTicket = (paid, booking) => {
    return new Promise((resolve, reject) => {
      ...
      db.collection('tickets').insertOne(payload, (err, ticket) => {
        if (err) {
          reject(new Error('an error occured registering a ticket, err:' + err))
        }
        resolve(payload)
      })
    })
  ...
  const getOrderById = (orderId) => {
    return new Promise((resolve, reject) => {
      ...
      const response = (err, order) => {
        if (err) {
          reject(new Error('An error occured retrieving a order, err: ' + err))
        }
        resolve(order)
      }
